      ...
      <div className="flex items-center gap-8 flex-col justify-center">
        {
          projects.length > 0 && projects.map((project) => (
            <div key={project.id} className="flex flex-col md:flex-row justify-center gap-4 items-center hover:scale-105 rounded-md hover:bg-gray-100 p-4">
              <img src={project.image} alt={project.imageAlt} 
              className="max-h-56 flex-shrink object-fill"
              />
              <div className="font-roboto pl-4">
                <h3 className="text-2xl font-bold text-blue-500 text-center">{project.title}</h3>
                <p className="text-center mobile-size">{project.description}</p>
